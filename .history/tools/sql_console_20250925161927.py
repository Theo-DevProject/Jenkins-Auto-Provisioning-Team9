...
app = Flask(__name__)   # <— this must exist so “from sql_console import app” works

def allowed(sql:str)->bool:
    s=sql.strip().lower()
    return s.startswith("select") and " insert " not in s and " update " not in s \
           and " delete " not in s and " drop " not in s and " alter " not in s and " create " not in s
...
if __name__ == "__main__":
    # Running directly also works if the service uses `python /opt/system-monitor/sql_console.py`

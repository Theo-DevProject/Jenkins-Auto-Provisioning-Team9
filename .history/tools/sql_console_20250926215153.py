...
def chart():
    ...
    # Expecting columns: id, timestamp, cpu_usage, memory_usage ... (index 2 & 3)
    try:
        cpu = sum(float(r[2]) for r in rows if r[2] is not None)
        mem = sum(float(r[3]) for r in rows if r[3] is not None)
    except Exception:
        return "Result set doesn't look like stats (need cpu/memory at cols 2/3). Run the default query first.", 400

    fig, ax = plt.subplots()
    ax.pie([cpu, mem], labels=["CPU Usage", "Memory Usage"], autopct='%1.1f%%')
    ax.set_title("CPU vs Memory Usage (sum over result set)")
